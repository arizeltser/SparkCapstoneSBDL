...
def test_read_accounts(spark):
    actual_df = DataLoader.read_accounts(spark, "LOCAL", False, None)
    assert actual_df.count() == 9
...
def test_read_addresses(spark):
    actual_df = DataLoader.read_addresses(spark, "LOCAL", False, None)
    assert actual_df.count() == 9


def test_account_to_contract(spark, expected_contract_df):
    accounts_df = DataLoader.read_accounts(spark, "LOCAL", False, None)
    actual_df = Transformations.account_to_contract(accounts_df)
    assert expected_contract_df.collect() == actual_df.collect()


def test_final_df(spark, expected_final_df):
    accounts_df = DataLoader.read_accounts(spark, "LOCAL", False, None)
    parties_df = DataLoader.read_parties(spark, "LOCAL", False, None)
    addresses_df = DataLoader.read_addresses(spark, "LOCAL", False, None)
    ...
    actual_df = Transformations.attach_header(spark, contract_parties_df).select("keys", "payload")


    print(expected_final_df.collect()[2][1])
    print("\n\n\n")
    print(actual_df.collect()[2][1])

    assert expected_final_df.collect() == actual_df.collect()
